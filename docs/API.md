...
| GET | `/:boardId/export.csv` | Выгрузка задач с учётом фильтров |
...
`GET /api/boards/:boardId/export.csv` принимает те же фильтры, что и список задач.
Файл отдаётся с BOM (иначе Excel на Windows ломает кириллицу), значения,
начинающиеся с `=`, `+`, `-` или `@`, экранируются — выгрузка не должна
превращаться в исполняемую формулу у того, кто её открыл.
